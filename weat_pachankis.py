...
pp = PdfPages('EngAll_WEAT_Pachankis.pdf')
...
if __name__ == '__main__':
	...
	#tablenames = ["vectors1800","vectors1810","vectors1990"]
	#tablenames = ["vectors1800","vectors1810"]

	# for table in tablenames:
	# 	with open("python_pachankis.txt", 'r') as f:
	# 		f.readline()  # skips header row
	# 		for line in f:
	# 			attribute = str(line.rstrip()).split() # changed here to add the "split()", we want the attribute text document to be read in as strings that are then split by spaces into individual stimuli
	...
	with open("python_pachankis.txt", 'r') as f:
		...
		for line in f:
			...
			try:
				...
				plt.title(attribute[0] +" vs good/bad")
	...
	df.to_csv("embeddings_pachankis_final.csv", index = False)
